...
export function getAccountNumberMobile(p_usuario: string, p_cuenta: string) {
  return new Promise((resolve) => {
    getSalesforceToken().then((token) => {
      getSalesforcePrimaryData(p_usuario, p_cuenta, token).then((res) => {
        resolve(res);
      });
    });
  });
...
async function getSalesforcePrimaryData(
  p_usuario: string,
  p_cuenta: string,
  p_token: string
) {
  ...
  if (
    ...
    retorno = GetPrimaryData4SkyCelularRespuesta.result.accountNumberMobile;
  }
  return retorno;
}
