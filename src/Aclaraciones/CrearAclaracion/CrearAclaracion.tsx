import styles from "./CrearAclaracion.module.css";
...
export const CrearAclaracion = () => {
  ...
  const helperList = [
    <>
      <span className="font-semibold">Escribenos a:</span>{" "}
      servicioenlinea@sky.com.mx
    </>,
  ];
      ...
      if (e.target.files) {
        ...
        if (files.length > 0) {
          ...
          if (file.size > 1048576) {
            setStatus("ERROR");
            setMessage("El archivo elegido pesa mas de 1MB!");
            setShowAlert(true);
            setTimeout(() => {
              setShowAlert(false);
            }, 3000);
  ...
  const doDisputeRequest = async () => {
    ...
    file_to_base64(file).then((result) => {
      ...
      setStatus("OK");
      setMessage(
        `Proceso de aclaracion ejecutado correctamente`);
      setShowAlert(true);
      setTimeout(() => {
        setShowAlert(false);
      }, 3000);
      //alert( `Proceso de aclaracion ejecutado correctamente`);
      setfileName("");
      setFileExtesion("");
      setFile(null);
      handleClose();
  ...
  return (
    <>
      ...
      <Row className={styles["headerRow"]}>
        <Col>
          <span className={styles["spectrum"]}>Aclaraciones</span>
        </Col>
      </Row>
      ...
      <Row className={styles["mynewbgcolor"]}>
        <Col>
          ...
          {helperList && (
            <ul className="unordered-list">
              {helperList.map((item, index) => {
                return (
                  <li key={`list-item-${index}`} className="list-item">
                    {item}
                  </li>
                );
              })}
          ...
          <p className={styles["subtitle"]}>
            El formato de PNG no está soportado, se recomienda enviar documentos
            en formato PDF que no superen 1 MB
          </p>
        </Col>
      </Row>
      ...
      <Row className={styles["subheaderRowTitle"]}>
        <Col>
          <div>
            <div className="input-required">Estado de cuenta</div>
            <div className="flex flex-row align-middle">
              <p className="flex flex-row border-2 rounded-md p-[.875rem] md:w-80 me-4">
                <Icon
                  className="align-[-0.125em]"
                  icon="ant-design:paper-clip-outlined"
                />
                {file ? fileName : "Adjuntar archivo"}
              </p>
              <input
                id="disputeFile"
                type="file"
                ...
                className="hidden"
              />
              <button className={styles.button} onClick={handleFileButtonClick}>
                {file ? "Actualizar" : "Adjuntar"}
              </button>
            </div>
            <div>
              <p>*</p>
              <label className="relative end-0 required-fields-label input-required">
                {" "}
                Campos obligatorios
              </label>
            </div>
