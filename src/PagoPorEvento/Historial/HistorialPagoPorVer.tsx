...
const HistorialPagoPorVer = (props: any) => {
  ...
  const builtComponentToReturn = (param: any) => {
    if (disputesSavedArray.length > 0) {
      return (
        <>
          <Container fluid className={styles["container"]}>
            <Row className={styles["subheaderRowGrid"]}>
              <Col xs={12} sm={12} md={12} lg={12} xl={12}>
                <p className={styles["title"]}>Tus Pagos por Evento Contratados</p>
              </Col>
            </Row>
          </Container>


          <div id="bigGrid" className={"d-none d-sm-block d-md-block d-lg-block d-xl-block"}>
            <Container fluid className={ styles["subheaderRowGrid"] + " " + styles["historialGrid"] }>
              <Row className={ styles["subheaderRowGrid"] + " " + styles["gridHeader"] } >
                <Col xs={2} sm={2} md={2} lg={2} xl={2} className={styles["adjustColWidthFecha"]} >
                  <span>Fecha</span>
                </Col>
                <Col xs={5} sm={5} md={5} lg={5} xl={5}>
                  <span>Titulo</span>
                </Col>
                <Col xs={2} sm={2} md={2} lg={2} xl={2} className={styles["adjustColWidthStatus"]}>
                  <span>Canal</span>
                </Col>
              ...
              {disputesSavedArray
                .map((item: any,index:number) => (
                  <Row className={ styles["subheaderRowGrid"] + " " + styles["grid"] } key={"bigRow_" +index}>
                    <Col
                      xs={2}sm={2}md={2}lg={2}xl={2}className={styles["adjustColWidthFecha"]}>
                      <span>{item.FECHA_ORDEN}</span>
                    </Col>
                    <Col xs={5} sm={5} md={5} lg={5} xl={5}>
                      <span>{item.EVENTO}</span>
                    </Col>
                    <Col xs={2} sm={2}  md={2}  lg={2} xl={2} className={ styles["alignToLeft"] + " " + styles["adjustColWidthStatus"] }>
      ...
      <div className={styles["aviso"]}>
        <p className={styles["avisoTitulo"]}>Parece que aún no has contratado ningún evento</p>
        <p className={styles["avisoTexto"]}>Cuando contrates uno podrás visualizarlo aquí</p>
       </div>
       </Container>    
       </>);
    }
...
export default HistorialPagoPorVer;
